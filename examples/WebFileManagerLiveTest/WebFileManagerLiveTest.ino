// ArduinoOPTA-FTPS - Opta Web File Manager Live Test
// SPDX-License-Identifier: CC0-1.0
//
// Hosts a small web UI on Arduino Opta for FTPS settings, transfer checks,
// and file-manager workflow prototyping without reflashing.
...
static const size_t kFsListPreviewLen = 640;

static const uint32_t kHttpReadTimeoutMs = 1500;

static const bool kEnableAuthGate = true;
static const char *kHarnessPasscode = "change-this-passcode";
...
static const IPAddress kStaticFallbackIp(192, 168, 7, 210);
static const IPAddress kStaticFallbackDns(192, 168, 7, 1);
static const IPAddress kStaticFallbackGateway(192, 168, 7, 1);
static const IPAddress kStaticFallbackSubnet(255, 255, 255, 0);
static const bool kRunBootSelfTest = false;
...
static char gLocalFsPreview[kFsListPreviewLen] = {};
static char gRemoteFsPreview[kFsListPreviewLen] = {};
...
<head>
  ...
  <title>Opta FTPS File Manager</title>
  <style>
    :root {
      --ink: #101820;
      --paper: #f6f2e8;
...
<body>
  <main class="shell">
    <header class="head">
      <h1>Opta FTPS File Manager Prototype</h1>
      <p>Use this LAN page to tune FTPS settings, run transfer checks, and prototype local/remote file-manager workflows.</p>
    </header>
    <section class="grid">
      <article class="card">
        ...
        <div class="fields">
          ...
          <div>
            <label for="fm-path">File Manager Path</label>
            <input id="fm-path" placeholder="/ftps_test">
          </div>
          <div>
            <label for="fm-target">Move/Copy Target</label>
            <input id="fm-target" placeholder="/ftps_test/new_name.txt">
          </div>
        </div>
        <div class="btns">
          ...
          <button id="btn-fs-local">Refresh Local</button>
          <button id="btn-fs-remote">Refresh Remote</button>
          <button id="btn-fs-mkdir">Create Remote Folder</button>
          <button id="btn-fs-delete">Delete Remote Path</button>
          <button id="btn-fs-move">Move Remote Path</button>
          <button id="btn-fs-copy">Copy Remote File</button>
        </div>
        <p class="note">ImportedCert mode expects a compile-time PEM in this sketch. Config updates use POST so credentials are not sent in the URL. Remote list/delete/move/copy are enabled. Local filesystem listing remains a scaffold and remote copy is buffer-limited for this demo.</p>
      </article>
      ...
      <article class="card">
        ...
        <label for="local-files" style="margin-top:10px; display:block;">Local Filesystem View (prototype)</label>
        <pre id="local-files"></pre>

        <label for="remote-files" style="margin-top:10px; display:block;">Remote FTPS View (prototype)</label>
        <pre id="remote-files"></pre>
      </article>
    </section>
  </main>
  ...
  <script>
    const s = {
      ...
      ids: {
        ...
        remoteFiles: document.getElementById('remote-files'),
        actionButtons: [
          ...
          document.getElementById('btn-fs-local'),
          document.getElementById('btn-fs-remote'),
          document.getElementById('btn-fs-mkdir'),
          document.getElementById('btn-fs-delete'),
          document.getElementById('btn-fs-move'),
          document.getElementById('btn-fs-copy')
        ]
      }
    };
    ...
    function renderStatus(data) {
      ...
      s.ids.localFiles.textContent = data.localFsPreview || 'Local listing not loaded yet.';
      s.ids.remoteFiles.textContent = data.remoteFsPreview || 'Remote listing not loaded yet.';
    ...
    async function doFsAction(op) {
      ...
      try {
        ...
        const p = new URLSearchParams();
        p.set('op', op);
        p.set('path', s.ids.fmPath.value.trim() || s.ids.path.value.trim());
        p.set('to', s.ids.fmTarget.value.trim());

        const fsRes = await fetch('/fs?' + p.toString(), {
          cache: 'no-store',
          headers: authHeaders()
        });

        if (fsRes.status === 401) {
          const authData = await fsRes.json();
          applyUnauthorized(authData.message || 'Auth required');
          return;
        }

        const fsData = await fsRes.json();
        if (fsData && typeof fsData.message === 'string') {
          s.ids.lastMessage.textContent = fsData.message;
        }
        if (fsData && typeof fsData.localFsPreview === 'string') {
          s.ids.localFiles.textContent = fsData.localFsPreview;
        }
        if (fsData && typeof fsData.remoteFsPreview === 'string') {
          s.ids.remoteFiles.textContent = fsData.remoteFsPreview;
        }
      } catch (error) {
        console.error(error);
      } finally {
    ...
    document.getElementById('btn-fs-local').addEventListener('click', () => doFsAction('local-list'));
    document.getElementById('btn-fs-remote').addEventListener('click', () => doFsAction('remote-list'));
    document.getElementById('btn-fs-mkdir').addEventListener('click', () => doFsAction('remote-mkdir'));
    document.getElementById('btn-fs-delete').addEventListener('click', () => doFsAction('delete'));
    document.getElementById('btn-fs-move').addEventListener('click', () => doFsAction('move'));
    document.getElementById('btn-fs-copy').addEventListener('click', () => doFsAction('copy'));
...
void resetConfigToDefaults() {
  ...
  copyStringToBuffer("Local listing not loaded yet.", gLocalFsPreview, sizeof(gLocalFsPreview));
  copyStringToBuffer("Remote listing not loaded yet.", gRemoteFsPreview, sizeof(gRemoteFsPreview));
}
...
bool appendLine(char *dest, size_t destSize, const char *line) {
  if (dest == nullptr || destSize == 0 || line == nullptr) {
    return false;
  }

  size_t used = strlen(dest);
  size_t left = (used < destSize) ? (destSize - used) : 0;
  if (left <= 1) {
    return false;
  }

  int n = snprintf(dest + used, left, "%s", line);
  return n > 0 && static_cast<size_t>(n) < left;
}

bool buildLocalFsListing(const char *requestedPath, char *out, size_t outSize) {
  if (out == nullptr || outSize == 0) {
    return false;
  }
  char path[kPathLen] = {};
  if (!copyStringToBuffer(hasValue(requestedPath) ? requestedPath : "/", path, sizeof(path))) {
    return copyStringToBuffer("Local path is invalid or too long.", out, outSize);
  }

  return snprintf(out,
                  outSize,
                  "Local listing is not enabled in this build for path=%s\n"
                  "(toolchain lacks dirent support on this target).\n"
                  "Remote listing/actions are fully functional.",
                  path) > 0;
}
...
void runBootSelfTest() {
  ...
  pushLog("Self-test: starting FTPS connect/list probe");
  Serial.println("[STEP] FTPS self-test connect/list");

  char message[kStatusMessageLen] = {};
  size_t bytesOut = 0;
  if (!runConnectAction(message, sizeof(message), bytesOut)) {
    pushLog("Self-test connect failed: %s", message);
    Serial.print("[FAIL] FTPS connect: ");
    Serial.println(message);
    return;
  }

  pushLog("Self-test connect OK");
  Serial.println("[PASS] FTPS connect");

  char err[192] = {};
  size_t listedBytes = 0;
  if (gFtps.list("/", gRemoteFsPreview, sizeof(gRemoteFsPreview), listedBytes, err, sizeof(err))) {
    pushLog("Self-test list OK bytes=%lu", static_cast<unsigned long>(listedBytes));
    Serial.print("[PASS] FTPS list bytes=");
    Serial.println(static_cast<unsigned long>(listedBytes));
  } else {
    pushLog("Self-test list failed: %s", err[0] ? err : "list() failed");
    Serial.print("[FAIL] FTPS list: ");
    Serial.println(err[0] ? err : "list() failed");
  }
...
void sendReportText(EthernetClient &client) {
  ...
  client.println(F("Opta FTPS Web File Manager Report"));
  client.println(F("============================"));
...
void sendStatusJson(EthernetClient &client, const char *statusCode = "200 OK") {
  ...
  client.print(F(",\"localFsPreview\":"));
  jsonPrintEscaped(client, gLocalFsPreview);

  client.print(F(",\"remoteFsPreview\":"));
  jsonPrintEscaped(client, gRemoteFsPreview);
...
void sendFsJson(EthernetClient &client,
                const char *statusCode,
                bool ok,
                const char *message) {
  sendHeader(client, statusCode, "application/json");
...
void handleFsRequest(const char *query, EthernetClient &client) {
  if (gStatus.busy) {
    sendFsJson(client, "409 Conflict", false, "Action is running; try again shortly.");
    return;
  }
  ...
  if (!readQueryValue(query, "op", op, sizeof(op))) {
    sendFsJson(client, "400 Bad Request", false, "Missing op parameter.");
    return;
  }

  if (!readQueryValue(query, "path", path, sizeof(path))) {
    copyStringToBuffer(gConfig.remotePath, path, sizeof(path));
  }
  readQueryValue(query, "to", to, sizeof(to));

  if (strcmp(op, "local-list") == 0) {
    if (!buildLocalFsListing(path, gLocalFsPreview, sizeof(gLocalFsPreview))) {
      pushLog("fs local-list failed path=%s", hasValue(path) ? path : "/");
      sendFsJson(client, "500 Internal Server Error", false, "Local listing failed.");
      return;
    }

    pushLog("fs local-list OK path=%s", hasValue(path) ? path : "/");
    sendFsJson(client, "200 OK", true, "Local listing retrieved.");
    return;
  }

  if (!gStatus.connected) {
    size_t connectBytes = 0;
    char connectMsg[kStatusMessageLen] = {};
    if (!runConnectAction(connectMsg, sizeof(connectMsg), connectBytes)) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "Remote action failed: %s",
               connectMsg[0] ? connectMsg : "connect() failed");
      sendFsJson(client, "500 Internal Server Error", false, gRemoteFsPreview);
      return;
    }
  }

  if (strcmp(op, "remote-list") == 0) {
    char err[192] = {};
    size_t listedBytes = 0;
    if (!hasValue(path)) {
      copyStringToBuffer("/", path, sizeof(path));
    }
    ...
    if (gFtps.list(path,
                   ...
                   err,
                   sizeof(err))) {
      pushLog("fs remote-list OK path=%s bytes=%lu",
              path,
              static_cast<unsigned long>(listedBytes));
      sendFsJson(client, "200 OK", true, "Remote listing retrieved.");
    } else {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "list failed path=%s\nerror=%s",
               path,
               err[0] ? err : "list() failed");
      sendFsJson(client, "500 Internal Server Error", false, "Remote listing failed.");
    }
    return;
  }

  if (strcmp(op, "remote-mkdir") == 0) {
    char err[192] = {};
    if (!hasValue(path)) {
      sendFsJson(client, "400 Bad Request", false, "Path required for remote-mkdir.");
      return;
    }

    if (!gFtps.mkd(path, err, sizeof(err))) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "mkd failed path=%s\nerror=%s",
               path,
               err[0] ? err : "mkd() failed");
      sendFsJson(client, "500 Internal Server Error", false, "Remote mkdir failed.");
      return;
    }

    snprintf(gRemoteFsPreview,
             sizeof(gRemoteFsPreview),
             "Created remote directory:\n%s\n\nUse Refresh Remote to list this path.",
             path);
    pushLog("fs remote-mkdir OK path=%s", path);
    sendFsJson(client, "200 OK", true, "Remote directory created.");
    return;
  }
  ...
  if (strcmp(op, "delete") == 0) {
    char err[192] = {};
    if (!hasValue(path)) {
      sendFsJson(client, "400 Bad Request", false, "Path required for delete.");
      return;
    }

    if (gFtps.dele(path, err, sizeof(err)) || gFtps.rmd(path, err, sizeof(err))) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "Deleted remote path:\n%s",
               path);
      pushLog("fs delete OK path=%s", path);
      sendFsJson(client, "200 OK", true, "Remote delete succeeded.");
      return;
    }

    snprintf(gRemoteFsPreview,
             sizeof(gRemoteFsPreview),
             "delete failed path=%s\nerror=%s",
             path,
             err[0] ? err : "dele()/rmd() failed");
    sendFsJson(client, "500 Internal Server Error", false, "Remote delete failed.");
    return;
  }

  if (strcmp(op, "move") == 0) {
    char err[192] = {};
    if (!hasValue(path) || !hasValue(to)) {
      sendFsJson(client, "400 Bad Request", false, "Both path and to are required for move.");
      return;
    }

    if (!gFtps.rename(path, to, err, sizeof(err))) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "move failed from=%s\nto=%s\nerror=%s",
               path,
               to,
               err[0] ? err : "rename() failed");
      sendFsJson(client, "500 Internal Server Error", false, "Remote move failed.");
      return;
    }
    ...
    snprintf(gRemoteFsPreview,
             ...
             path,
             to);
    pushLog("fs move OK from=%s to=%s", path, to);
    sendFsJson(client, "200 OK", true, "Remote move succeeded.");
    return;
  }
  ...
  if (strcmp(op, "copy") == 0) {
    char err[192] = {};
    if (!hasValue(path) || !hasValue(to)) {
      sendFsJson(client, "400 Bad Request", false, "Both path and to are required for copy.");
      return;
    }

    size_t remoteBytes = 0;
    if (!gFtps.size(path, remoteBytes, err, sizeof(err))) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "copy preflight failed size path=%s\nerror=%s",
               path,
               err[0] ? err : "size() failed");
      sendFsJson(client, "500 Internal Server Error", false, "Remote copy preflight failed.");
      return;
    }

    if (remoteBytes > kDownloadBufferSize) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "copy path=%s\nsize=%lu exceeds demo buffer=%u\nUse smaller file or raise kDownloadBufferSize.",
               path,
               static_cast<unsigned long>(remoteBytes),
               static_cast<unsigned>(kDownloadBufferSize));
      sendFsJson(client, "413 Payload Too Large", false, "Copy file exceeds demo buffer.");
      return;
    }

    uint8_t buffer[kDownloadBufferSize] = {};
    size_t bytesRead = 0;
    if (!gFtps.retrieve(path, buffer, sizeof(buffer), bytesRead, err, sizeof(err))) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "copy retrieve failed path=%s\nerror=%s",
               path,
               err[0] ? err : "retrieve() failed");
      sendFsJson(client, "500 Internal Server Error", false, "Remote copy retrieve failed.");
      return;
    }

    if (!gFtps.store(to, buffer, bytesRead, err, sizeof(err))) {
      snprintf(gRemoteFsPreview,
               sizeof(gRemoteFsPreview),
               "copy store failed to=%s\nerror=%s",
               to,
               err[0] ? err : "store() failed");
      sendFsJson(client, "500 Internal Server Error", false, "Remote copy store failed.");
      return;
    }
    ...
    pushLog("fs copy OK from=%s to=%s bytes=%lu",
            path,
            to,
            static_cast<unsigned long>(bytesRead));
    sendFsJson(client, "200 OK", true, "Remote copy succeeded.");
    return;
  }

  sendFsJson(client, "400 Bad Request", false, "Unknown fs operation.");
}
...
void handleClient(EthernetClient &client) {
  ...
  } else if (strcmp(target, "/fs") == 0) {
    if (strcmp(method, "GET") == 0) {
      handleFsRequest(query, client);
    } else {
      sendTextResponse(client, "405 Method Not Allowed", "Use GET /fs?op=...");
    }
...
void setup() {
  ...
  Serial.println();
  Serial.println("==============================================");
  Serial.println("  Opta FTPS Web File Manager Live Test");
  Serial.println("==============================================");
  ...
  if (Ethernet.begin(mac) == 0) {
    Serial.println("[WARN] Ethernet.begin(): DHCP failed. Trying static fallback 192.168.7.210");
    pushLog("Ethernet.begin() DHCP failed; trying static fallback 192.168.7.210");
    Ethernet.begin(mac,
                   kStaticFallbackIp,
                   kStaticFallbackDns,
                   kStaticFallbackGateway,
                   kStaticFallbackSubnet);
  }

  if (Ethernet.localIP() == IPAddress(0, 0, 0, 0)) {
    setOutcome("startup", false, FtpsError::NetworkNotInitialized, "Ethernet initialization failed.", 0, 0);
    pushLog("Ethernet init failed; web harness is unavailable.");
    Serial.println("[FAIL] Ethernet initialization failed.");
    return;
  }
  ...
  setOutcome("startup", true, FtpsError::None, "Web file manager ready.", 0, 0);

  Serial.println("[PASS] Web file manager started");
